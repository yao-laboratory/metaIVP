...
def determine_contigs(contigs_fasta_file,quality_summary,genomad_file,output_directory):
    ...
    ########
    ########
    #new code for genomad....
    contigs_virus_summary_genomad=pd.read_csv(genomad_file, sep="\t")
    #"/work/yaolab/shared/2023_06_metaIVP/scripts/bin_contig_level_genomad/contigs_summary/contigs_virus_summary.tsv"
    print(contigs_virus_summary_genomad.shape)
    contigs_virus_summary_genomad=contigs_virus_summary_genomad.rename(columns={"seq_name": "contig_id"})

    common_virus_genomad_checkv=pd.merge(not_determined_df,contigs_virus_summary_genomad,on='contig_id',how='inner')
    contig_list_genomad=common_virus_genomad_checkv['contig_id'].tolist()
    genomad_good_df=not_determined_df[not_determined_df['contig_id'].isin(contig_list_genomad)]
    new_low_medium_high_df=pd.concat([low_medium_high_df, genomad_good_df], ignore_index = True)
    new_not_determined_df=not_determined_df[~not_determined_df['contig_id'].isin(contig_list_genomad)]

    contig_fasta_summary_file_determined = pd.merge(contig_fasta_df, new_low_medium_high_df[['contig_id', 'checkv_quality']], on=['contig_id','checkv_quality'],how='inner')

    contig_fasta_summary_file_not_determined = pd.merge(contig_fasta_df, new_not_determined_df[['contig_id', 'checkv_quality']], on=['contig_id','checkv_quality'], how='inner')
#added another parameter for merge. need to test
    ...
    # Open a FASTA file to write the output
    fasta_file_path_determined = os.path.join(output_directory,"contigs_determined.fasta")
    fasta_file_path_not_determined=os.path.join(output_directory,"contigs_not_determined.fasta")

    print("fasta files which will contain contigs determined and not determined are")
    print(fasta_file_path_determined)
    print(fasta_file_path_not_determined)

    #for determined
    with open(fasta_file_path_determined, 'w') as fasta_file:
        for index, row in contig_fasta_summary_file_determined.iterrows():
            contig_id = row['contig_id']
            sequence = row['seq']
        # Write in FASTA format
            fasta_file.write(f'>{contig_id}\n')
            fasta_file.write(f'{sequence}\n')

    #for non detmined

    with open(fasta_file_path_not_determined, 'w') as fasta_file:
        for index, row in contig_fasta_summary_file_not_determined.iterrows():
            contig_id = row['contig_id']
            sequence = row['seq']
        # Write in FASTA format
            fasta_file.write(f'>{contig_id}\n')
...
def determine_contigs_old(contigs_fasta_file,quality_summary,output_directory):
...
def determine_bins(bins_scaffold_file,bin_fasta,bin_quality_checkv_summary,contig_fasta_summary_file_determined,contig_fasta_summary_file_not_determined,output_directory):
    print("This is deterimine bins function")
    print("************************""""""""""""""""""""""""""""""""")
    print(bins_scaffold_file)
    print(bin_fasta)
    print(bin_quality_checkv_summary)
    print(contig_fasta_summary_file_determined)
    ...
    not_determined_bin_fasta_file=os.path.join(output_directory,"bins_not_virus_part1.fasta")
    ...
    #keep all good bins with contigs
    bin_fasta_summary_file_determined=bin_fasta_summary_file_determined['bin_id']
    bins_scaffold_all=pd.merge(bins_scaffold,bin_fasta_summary_file_determined, on='bin_id', how='inner')
   
    bins_scaffold=pd.merge(bins_scaffold_all,contig_fasta_summary_file_determined, on='contig_id', how='inner')
    #determined bin summary and  virus bin scaffold file
    #virus contigs belonging to which viral bins
    ...
    #this is part 2
    #for removed contigs from the non-viral part
    contig_fasta_summary_file_not_determined = pd.read_csv(contig_fasta_summary_file_not_determined,sep=',',header=0)

    bins_scaffold=pd.merge(bins_scaffold_all,contig_fasta_summary_file_not_determined, on='contig_id', how='inner')

    bins_scaffold[['contig_id','bin_id']].to_csv(os.path.join(output_directory,"not_viral_bin_scaffold.csv"),sep=',',header=True, index=None)

    unique_bin_id=bins_scaffold['bin_id'].unique()

    cleaned_bin_fasta_filename=os.path.join(output_directory,"bins_not_virus_part2.fasta")
    cleaned_bin_fasta_file = open(cleaned_bin_fasta_filename, 'w')
    for bin_id in unique_bin_id:
        current_bins_scaffold=bins_scaffold[bins_scaffold['bin_id']==bin_id]
        cleaned_bin_fasta_file.write(">"+bin_id+"\n")
        ...
        cleaned_bin_fasta_file.write("\n")
    cleaned_bin_fasta_file.close()
    #remember to combine part1 and part2 to do other analysis
...
def main():
    ...
    determine_contig_parser.add_argument("-g", dest="Genomad_file",type=str, help="genomad file")
    ...
    determine_bin_parser.add_argument("-nd", dest="Contig_fasta_summary_file_not_determined", type=str, help="/path/to/contig_fasta_summary_file_non_determined.csv")
    ...
    if args.subcommand=='contigs_determine':
        ...
        genomad_file = args.Genomad_file
        output_directory = args.Output_file_path
        print(input_contig_fasta,quality_summary,genomad_file,output_directory)
        determine_contigs(input_contig_fasta,quality_summary,genomad_file,output_directory)
        ...
        if args.subcommand=='bin_determine':
            ...
            contig_fasta_summary_file_not_determined=args.Contig_fasta_summary_file_not_determined
            output_directory = args.Output_file_path
            print(input_bin_fasta,bins_scaffold_file,bin_quality_checkv_summary,contig_fasta_summary_file_determined,contig_fasta_summary_file_not_determined,output_directory)
            determine_bins(bins_scaffold_file,input_bin_fasta,bin_quality_checkv_summary,contig_fasta_summary_file_determined,contig_fasta_summary_file_not_determined,output_directory)
        else:
